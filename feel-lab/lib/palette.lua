...
local HEX = {
  ...
  -- ── Extension VFX D'ATTAQUE (feel-lab) : highlights VIFS de chaque type de dégât, calés EXACTEMENT sur
  -- Theme.c afflictions (déjà validés dans le jeu) pour que les sprites d'attaque parlent le même langage
  -- chromatique que affliction_fx/affliction_icons. Plus saturés que le corps de la palette (ce sont des
  -- POINTES de VFX additif, pas de la chair) mais restent désaturés Wraeclast. Append-only.
  J = 0xf2d24a, -- choc : jaune ÉLECTRIQUE (= Theme.c.shock ; cœur d'éclair, pointe d'étincelle)
  k = 0xe0792e, -- burn : braise ORANGE vive (= Theme.c.burn ; corps de flamme/projectile)
  j = 0x93c12f, -- poison : vert TOXIQUE vif (= Theme.c.poison ; corps de glob/splat)
  u = 0xd8475e, -- bleed : cramoisi VIF (= Theme.c.bleed ; goutte/giclée vive)
  U = 0x6a1414, -- bleed sombre (= Theme.c.bleedDeep ; flaque, ombre de gerbe)
  w = 0xa86fc4, -- rot : violet NÉCROTIQUE clair (= Theme.c.rot ; corps de pustule/spore)
}
