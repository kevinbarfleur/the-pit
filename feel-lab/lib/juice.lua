-- feel-lab/lib/juice.lua
-- LA COUCHE "CANDY" — ce que Feel n'émet volontairement PAS (il est calibré grimdark : lift/glow, pas de
-- scale). Le lab expérimente le feel « bonbon sucré » des refs (Balatro/Tiny Rogue) : punch de SCALE en
-- ressort (squash & stretch), TILT, SCREEN-SHAKE trauma² (Eiserloh/Vlambeer) via love.math.noise, et HITSTOP.
-- 100% RENDER/cosmétique, headless-safe (love.math.noise existe aussi headless ; sinon repli sinus).
--
...
--
-- API :
--   Juice.update(dt)                 -- dt en SECONDES (ressorts framerate-correct + decay trauma + hitstop)
...
--   Juice.shake() -> dx, dy, rot     -- secousse caméra courante (en px DESIGN + radians)
--   Juice.freeze(sec)                -- hitstop (ne s'empile pas : garde le plus long)
--   Juice.timeScale() -> 0|1         -- à multiplier au dt de gameplay (le lab gèle le monde, pas l'UI)
--   Juice.reset()
...
-- bruit dans [-1,1] : love.math.noise(∈[0,1]) recentré ; repli sinus si love absent (headless).
