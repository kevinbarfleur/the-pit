-- src/ui/particles.lua  (PARTICULES PIXEL — transplant fidèle de feel-lab/lib/particles.lua)
...
--
-- Source de vérité : feel-lab/lib/particles.lua (démo validée du Feel Lab). On NE RÉ-INVENTE PAS : on
-- transplante. SEUL ÉCART avec le lab : les requires pointent vers les VRAIS modules du jeu (src.core.sprite,
-- src.core.palette) au lieu de lib.*. VÉRIFIÉ : src/core/sprite.lua a la MÊME signature (bake(grid,palette) ->
-- {image,w,h}) que feel-lab/lib/sprite.lua ; les 11 clés de char des grilles (W/Q/q/o/T/S/Y/s/A/a/y) existent
-- TOUTES dans src/core/palette.lua (Wraeclast réelle) -> aucun remap nécessaire, look identique.
--
