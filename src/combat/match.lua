...
--   opts = { tickCap=8000, attach?(arena)->handle, expose?, fatigue?, hpMult?, cooldownMult?, judge=true, assertPure? }
...
function Match.run(left, right, seed, opts)
  ...
  local arena = Arena.new({ left = left, right = right, autoReset = false, seed = seed,
    fatigue = opts.fatigue, hpMult = opts.hpMult, cooldownMult = opts.cooldownMult })
