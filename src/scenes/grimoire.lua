...
-- LE GRIMOIRE (pilier #2, méta-progression) : codex PERSISTANT des reliques. Une relique IDENTIFIÉE (par
-- déduction en run, cf. src/core/grimoire) y est inscrite à VIE au niveau du compte — sa connaissance
-- devient permanente (façon Obra Dinn). Les non identifiées restent CRYPTIQUES (« the ink runs here »).
--
...
function Screen:drawOverlay(view)
  ...
  for i, row in ipairs(self.rows) do
    ...
    local tcol = row.known and Theme.type(RELIC_TYPE[row.id] or "bone").color or c.lock
    Draw.pip(RELIC_TYPE[row.id] or "bone", r.x + 22, r.y + r.h / 2, 7, tcol)
